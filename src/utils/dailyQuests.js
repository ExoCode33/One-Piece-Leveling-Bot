...
class DailyQuests {
    ...
    async updateQuestProgress(userId, guildId, questType, increment = 1, metadata = {}) {
        try {
            ...
            if (questType === 'early_bird' || questType === 'night_owl') {
                ...
                // Check if current time is within the quest time window
                if (questType === 'early_bird' && (estHour < timeWindow.start || estHour >= timeWindow.end)) {
                    return false; // Not in early bird window (0-10 AM EST)
                }
                if (questType === 'night_owl' && (estHour < timeWindow.start)) {
                    return false; // Not in night owl window (10 PM+ EST)
                }
            }
            ...
            if (questType === 'social_butterfly' && metadata.channelId) {
                ...
                if (!channels.includes(metadata.channelId)) {
                    ...
                    increment = channels.length; // Set progress to total unique channels
                    ...
                    return false; // Channel already counted
                }
            }

            // Handle channel_explorer quest (track unique text channels for messages)
    ...
    async checkAllQuestsCompleted(userId, guildId) {
        try {
            ...
            const allCompleted = quests.rows.every(quest => quest.completed);
            
            if (allCompleted) {
                // Check if already awarded
                const existing = await this.db.query(`
                    SELECT * FROM daily_quest_completions 
                    WHERE user_id = $1 AND guild_id = $2 AND date = $3
                `, [userId, guildId, today]);

                if (existing.rows.length > 0) return; // Already awarded
                ...
                console.log(`[DAILY QUESTS] User ${userId} completed all daily quests!`);
            }

        } catch (error) {
            console.error('[DAILY QUESTS] Error checking all quests completed:', error);
        }
    }
    ...
    // Post all quests completed announcement
    async postAllQuestsCompleted(userId, guildId) {
        try {
            const questChannelId = process.env.DAILY_QUEST_CHANNEL;
            if (!questChannelId || questChannelId.includes('channel_id')) return;
            ...
            console.error('[DAILY QUESTS] Error posting all quests completion:', error);
        }
    }

    // Daily cleanup
    async cleanupDailyQuests() {
        try {
            console.log('[DAILY QUESTS] Starting daily cleanup...');
            
            // Clean up old quests (keep last 7 days)
            const questResult = await this.db.query(
                "DELETE FROM daily_quests WHERE date < CURRENT_DATE - INTERVAL '7 days'"
            );

            const completionResult = await this.db.query(
                "DELETE FROM daily_quest_completions WHERE date < CURRENT_DATE - INTERVAL '7 days'"
            );

            console.log(`[DAILY QUESTS] Cleaned up ${questResult.rowCount || 0} old quests and ${completionResult.rowCount || 0} old completions`);

        } catch (error) {
            console.error('[DAILY QUESTS] Error during cleanup:', error);
        }
    }
}
