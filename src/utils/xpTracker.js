    ...
    async logXPActivity(type, user, guildId, xpGain, additionalInfo = {}) {
        try {
            ...
            if (type === 'voice' || type === 'voice_silent') {
                ...
                if (member) {
                    const capInfo = this.getUserDailyXPCap(member);
    ...
    async getDailyVoiceXPStats(userId, guildId, days = 7) {
        try {
            ...
            if (guild) {
                const member = await guild.members.fetch(userId).catch(() => null);
                if (member) {
                    currentCap = this.getUserDailyXPCap(member);
                }
            }
...
module.exports = XPTracker;// src/utils/xpTracker.js - Complete Marine Intelligence XP Tracking System with Separated Buff/Cap Roles
...
class XPTracker {
        ...
        for (const [userId, session] of this.voiceSessions.entries()) {
            try {
                ...
                if (newDailyTotal > dailyCap) {
                    ...
                    hitCap = true;
                    console.log(`[DAILY CAP] ${user.username}: ${finalXP} → ${actualXPGain} (would exceed ${capInfo.source} cap: ${newDailyTotal}/${dailyCap})`);
                }
                ...
                console.log(`[VOICE XP] ${user.username}: +${actualXPGain} XP (Daily: ${updatedDailyXP}/${dailyCap} ${capInfo.source}) ${hitCap ? '[CAP HIT]' : ''}`);
    ...
    // Enhanced voice XP summary with ALL RED THEME and dynamic cap information
